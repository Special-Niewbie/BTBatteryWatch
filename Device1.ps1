# Script per verificare la connessione di un dispositivo Bluetooth
# Usage: .\Device1.ps1 "Nome Dispositivo"
...
if ([string]::IsNullOrEmpty($DeviceName)) {
    ...
    if (Test-Path $JsonPath) {
        try {
            ...
            # Estrai il nome del dispositivo dal JSON (device 1)
            if ($jsonContent -match '"device":\s*"1"[^{]*"name":\s*"([^"]*?)"') {
...
$outputPath = Join-Path -Path $PSScriptRoot -ChildPath "tempPSOutput_Connected.txt"
